      ...
      <div className="flex flex-col row-start-1 row-span-7 col-start-3 col-span-6 ml-5 mb-10">
        ...
        <table className="mt-4 text-center mr-10 table-auto border-collapse border border-slate-300">
          <thead className="grid grid-cols-16 font-bold text-lg rounded-[4px]">
            ...
            <th className=" border border-slate-300 col-span-1 p-1">
              Diện tích
            </th>
          ...
          <tbody className="text-sm">
            {posts.totalPosts ? (
              posts.result.map((post, idx) => {
                return (
                  <tr
                    className="grid grid-cols-16 rounded-[4px]"
                    key={idx}
                  >
                    ...
                    <td className=" border border-slate-300 col-span-1 p-1">
                      {post.area}
                    </td>
                    <td className=" border border-slate-300 col-span-2 p-1">
                      {post.price}
                    ...
                    <td className=" border border-slate-300 col-span-3 p-1">
                      {post.full_address.district +
                        ', ' +
                        post.full_address.city}
                    </td>
                    <td className=" border border-slate-300 col-span-3 p-1">
                      {post.createDateAgo}
                    </td>
              ...
              <div className="py-2">
                ...
                <a className="text-[#0000ff]" href="../post-new">
                  vào đây
                </a>{' '}
                để bắt đầu đăng tin
              </div>
